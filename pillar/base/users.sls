users:
  # Admins
  ee:
    fullname: "Ee Durbin"
    admin: True
    ssh_keys:
      - ssh-rsa 
      - ssh-rsa  ewd3yubikey2018
      - ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBM2rMn4DfxYVfJRdELytrMcNhUqDB7196YDrZsNoC+HxE62yiaS/fE9pn781L1KqcEUgpHjucFuP7ccox3qbc2E= ewdurbin@ewd3mbp
      - ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBBnWofdFIY68N19Q04fiWKPYx+dcpc7nmU10IyOyon2KJa5blCDoxFOemOeWl8doAfFO2qhprQ2YhNABes3pZOc= ee@chungus

  chloe:
    fullname: "Chloe Gerhardson"
    admin: True
    ssh_keys:
      - ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBCGR9HmTHRbpGL3fSGG8f9XvAvmymfb5WZTgiIskvmpc054SHV/9sbq13toPcJOFtxQ1Gk33xAPcMohbI6rJ0Vw= Chloes-MacBook-Pro@secretive.Chloe’s-MacBook-Pro.local

  dstufft:
    fullname: "Donald Stufft"
    admin: True
    shell: /bin/zsh
    packages:
      - zsh
      - git
    dotfiles: True
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG7R8y2Ecocj+t5rP7YNmNWM+QmmZ8Wuz0f1btJ3KcA0 donald@stufft.io
      - ssh-rsa  donald@stufft.io

  benjaminws:
    fullname: "Benjamin W. Smith"
    admin: True
    ssh_keys:
      - ssh-rsa  benjaminws@macbuntu
      - ssh-rsa  bsmith@bsmith-laptop

  coderanger:
    fullname: "Noah Kantrowitz"
    admin: True
    ssh_keys:
      - ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAIEAvV0vQo3OpzqDFDBHW5o5abdbNJNNg9YkiawTpSnOusB+E9Hp2Pae1jT3r+7ZUqIIutXuNOPOefIy6oR0YuZhL6d9uhRIl9LMTB0XXzj5aW5ZbbGO6nsaERQUU3ALYzrwxgX8kLvCcAQmrVhhRH88Zqo4lkRkxXpN1LdPANh9qa0= coderanger@alfred

  benjamin:
    fullname: "Benjamin Peterson"
    admin: True
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIC5RZqb06JuJrCClQkFvR/6rSvtFs9MSh7qQxe6gzVpz Benjamin Peterson
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDSSjSCl8VkJWWNLCBizhHOnC8jEjVbt7EDrOzxGJurM
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOZr7AjOkEWiJnP2hzmt5tZS53ST3O2WyRyiSXFT0Rde
    access:
      docs:
        allowed: True
        groups:
          - docs
      downloads:
        groups:
          - downloads

  # Other users
  antoine:
    fullname: "Antoine Pitrou"
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIECocvLw5VQdrDP6WdzVtitcB3qP5vkwVAaUvJWYwRp/ antoine@fsol
    access:
      hg:
        allowed: True
        sudo: True

  barry:
    fullname: "Barry Warsaw"
    ssh_keys:
      - ssh-rsa  mobile@Digital-Man
      - ssh-rsa  Keys for VMs
    access:
      docs:
        allowed: True
        groups:
          - docs
      downloads:
        allowed: True
        groups:
          - downloads
      mail:
        allowed: True
        sudo: True
      gnumailman:
        allowed: True
        sudo: True

  gbrandl:
    fullname: "Georg Brandl"
    ssh_keys:
      - ssh-rsa  gbr@georg
    access:
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      hg:
        allowed: True
        sudo: True

  larry:
    fullname: "Larry Hastings"
    ssh_keys:
      - ssh-rsa  larry@hastings.org
    access:
      docs:
        allowed: True
        groups:
          - docs
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      hg:
        allowed: True
        sudo: True

  loewis:
    fullname: "Martin von Loewis"
    ssh_keys:
      - ssh-dss  martin@mira
      - ssh-dss  loewis@kosh
      - ssh-dss  loewis@creosote.python.org
    access:
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads

  nad:
    fullname: "Ned Deily"
    ssh_keys:
      - ssh-rsa  ned.deily
    access:
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
          - docsbuild

  sdower:
    fullname: "Steve Dower"
    ssh_keys:
      - ssh-rsa 
    access:
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads

  brian:
    fullname: "Brian Curtin"
    ssh_keys:
      - ssh-rsa 
    access:
      cdn-logs:
        allowed: True

  zware:
    fullname: "Zachary Ware"
    ssh_keys:
      - ssh-rsa  zach@screamer
    access:
      codespeed:
        allowed: True
        sudo: True
      buildbot:
        allowed: True
        sudo : True

  haypo:
    fullname: "Victor Stinner"
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEIVzct/vFIVEAQ5aG8WJlJJWPNZYbVDyI5jwljst8mk haypo2017
    access:
      codespeed:
        allowed: True
        sudo: True
      buildbot:
        allowed: True
        sudo : True

  pablogsal:
    fullname: "Pablo Galindo"
    ssh_keys:
      - ssh-rsa 
      - ssh-rsa 
      - ssh-rsa 
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIwJt1t9vGbHVnzcsiFXWEFVS/LgZCbvk7YbZGVHGd2q
      - sk-ecdsa-sha2-nistp256@openssh.com AAAAInNrLWVjZHNhLXNoYTItbmlzdHAyNTZAb3BlbnNzaC5jb20AAAAIbmlzdHAyNTYAAABBBPGSxFq7F3Gw01OpBv8nyh09qGo/92ms6DqyziJX9difzWUIMJlS+rT9BlHTkb7D8p7hRS/8JYnKWQGbVs69ZMwAAAAEc3NoOg== ssh:
    access:
      codespeed:
        allowed: True
        sudo: True
      buildbot:
        allowed: True
        sudo : True
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
          - docsbuild

  isidentical:
    fullname: "Batuhan Taskaya"
    ssh_keys:
      - ssh-rsa 
    accesss:
      codespeed:
        allowed: True
        sudo: True

  jafo:
    fullname: "Sean Reifschneider"
    ssh_keys:
      - ssh-rsa  jafo@guin
    access:
      mail:
        allowed: True

  msapiro:
    fullname: "Mark Sapiro"
    ssh_keys:
      - ssh-dss  msapiro@msapiro
      - ssh-rsa AAAAB3NzaC1yc2EAAAABIwAAAIEArQw24fE8BZmcrtxwuiMOC3g5YXwi1YVLNUkhVSwrHyhvlcu5jRP7pj05pK1naEz8MQ4crNnrhTAgcpMJUAyJgQ/y0a9qrRWO4H+hVtWpMTgiGFsscoKWdu0xTnlK3EbV/vBeMPnSd0usXR3E0+I6GlWJ4/SmtCx0OwqEtfduZV8= msapiro@msapiro
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGjnfQcLTNj0eXqDkIQo75lXiZw+CJDWSHCD70Ao7P7q mark@msapiro
    access:
      mail:
        allowed: True
        sudo: True
        groups:
          - mailman
      gnumailman:
        allowed: True
        sudo: True
        groups:
          -mailman

  hildeb:
    fullname: "Ralf Hildebrandt"
    ssh_keys:
      - ssh-rsa  hildeb@senftopf
    access:
      mail:
        allowed: True
        sudo: True

  skip:
    fullname: "Skip Montanaro"
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJA80VH5sZrqYnsFzUiUTKT4Wv7NsfOBIjxCyoHM14L1 skip@montanaro.dyndns.org
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJ9EM3GdblHxAdPCtSdFuDO2/8GKhDjtHpdtP+sXfF0F skip@skips-mbp.lan
    access:
      mail:
        allowed: True
        sudo: True

  mdk:
    fullname: "Julien Palard"
    ssh_keys:
      - ssh-rsa  julien+yubikey@palard.fr
      - ssh-rsa  julien+yubikey5@palard.fr
    access:
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
          - docsbuild

  maxking:
    fullname: "Abhilash Raj"
    ssh_keys:
      - ssh-rsa  maxking@angel
      - ssh-rsa  phoenix
      - ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBMV/+IM81xKIyZhJHnCqc6mFBQjKxBj0sEx3Gj3N/DPXL1NXtf49Dj/Wx44AKy5fda+mVZZTzQVB9C7F0X+kRJE= maxking@Abhilashs-MacBook-Pro.local
    access:
      mail:
        allowed: True
        sudo: True
        groups:
          - mailman
      gnumailman:
        allowed: True
        sudo: True
        groups:
          - mailman

  ambv:
    fullname: "Łukasz Langa"
    ssh_keys:
      - ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMGC2VGofArUyzGR25TbLFtk16bYT+6TuCHui7qgSxvp lukasz.langa
    access:
      codespeed:
        sudo: True
        allowed: True
      buildbot:
        sudo: True
        allowed: True
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
          - docsbuild
      bugs:
        sudo: True
        allowed: True
        groups:
          - roundup

  mattip:
    fullname: "Matti Picus"
    ssh_keys:
      - ssh-rsa  matti@asus
      - ssh-rsa  mattip@tannit
    access:
      codespeed:
        sudo: True
        allowed: True
      pypy-web:
        sudo: True
        allowed: True

  fijal:
    fullname: "Maciej Fijalkowski"
    ssh_keys:
      - ssh-dss  fijal@zygmunt
    access:
      codespeed:
        sudo: True
        allowed: True
      pypy-web:
        sudo: True
        allowed: True

  ezio:
    fullname: "Ezio Melotti"
    ssh_keys:
      - ssh-rsa  ezio-melotti
    access:
      bugs:
        sudo: True
        allowed: True
        groups:
          - roundup

  rouilj:
    fullname: "John Rouillard"
    ssh_keys:
      - ssh-rsa  rouilj@issues
    access:
      bugs:
        sudo: True
        allowed: True
        groups:
          - roundup

  schlatterbeck:
    fullname: "Ralf Schlatterbeck"
    ssh_keys:
      - ssh-rsa  rsc@runtux.com
    access:
      bugs:
        sudo: True
        allowed: True
        groups:
          - roundup

  ammar:
    fullname: "Ammar Askar"
    ssh_keys:
      - ssh-rsa  ammar_laptop
    access:
      bugs:
        sudo: True
        allowed: True
        groups:
          - roundup

  lemburg:
    fullname: "Marc-Andre Lemburg"
    ssh_keys:
      - ssh-rsa  marc-andre.lemburg
    access:
      moin:
        sudo: True
        allowed: True
        groups:
          - moin
      planet:
        sudo: True
        allowed: True

  thomas:
    fullname: "Thomas Wouters"
    ssh_keys:
      - ssh-rsa  thomas@python.org
      - ssh-rsa  twouters@google.com
    access:
      buildbot:
        allowed: True
        sudo : True
      downloads:
        sudo: True
        allowed: True
        groups:
          - downloads
      docs:
        sudo: True
        allowed: True
        groups:
          - docs
          - docsbuild
